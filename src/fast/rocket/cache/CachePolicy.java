 ...
 * This policy will only retrieve data from the cache, and will not use any network connection.Use this policy in 
 * combination with another policy, to allow for quick response times without requiring a network connection for 
 * specific operations.

 * CachePolicy.CACHEFIRST
 * This policy will first attempt to retrieve data from the cache. If the data has been cached, it will be returned. 
 * If the data does not exist in the cache, the data will be retrieved from server and the cache will be updated.
 * Use this policy if your application can display data that doesn't change very often but you still want local updates.

 * CachePolicy.CACHEFIRST_NOREFRESH
 * This policy will first attempt to retrieve data from the cache. If the data has been cached, it will be returned. 
 * If the data does not exist in the cache, the data will be retrieved from server but the cache will not be updated 
 * with the new results.Use this policy if you want to set default results, however if a request is made that cannot 
 * return these defaults a live request will be made (without modifying those default values)
